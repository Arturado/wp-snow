...
// Portfolio: eventos pasados ordenados por fecha DESC
$snow_query_portfolio = new WP_Query([
    'post_type'      => 'evento',
    'posts_per_page' => -1,
    'meta_key'       => '_evento_fecha',
    'orderby'        => 'meta_value',
...
$snow_portfolio_posts = $snow_query_portfolio->posts;
...
$portfolio_visible = 6;
$portfolio_total   = $snow_query_portfolio->found_posts;
$portfolio_extra   = max(0, $portfolio_total - $portfolio_visible);
    ...
    <div class="container">
      <div class="section-header">
        <div>
          ...
          <p class="section-subtitle">Portafolio &middot; más de 1.000 shows</p>
        </div>
      </div>

      <div class="portfolio-grid" id="portfolio-grid">
        <?php $index = 0;
        foreach ($snow_portfolio_posts as $post) :
          setup_postdata($post);
          $id         = $post->ID;
          $titulo     = get_the_title($id);
          $ciudad     = get_post_meta($id, '_evento_ciudad', true);
        ...
        ?>
        <div
          class="portfolio-card<?php echo $index >= $portfolio_visible ? ' portfolio-card--hidden' : ''; ?>"
          style="background-image: url('<?php echo esc_url($imagen); ?>')"
          role="img"
          aria-label="<?php echo esc_attr($titulo); ?>"
        >
